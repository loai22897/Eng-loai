...
import { Message, InvoiceData, PartLookupData, FaultRecord } from '../types';
import { streamChatResponse, analyzeMultimodal } from '../services/geminiService';
import InvoiceCard from './InvoiceCard';
import { Bookmark, Save, Sparkles, Send, Paperclip, Mic, MicOff, Camera, User, Bot, AlertCircle } from 'lucide-react';
    ...
    setMessages([
      {
        ...
        type: 'text',
        content: `${greeting}
أنا مساعدك الذكي (AI Print) لصيانة الطابعات.

يمكنني مساعدتك في:
🔹 تشخيص الأعطال بالصور أو الصوت.
🔹 العثور على **روابط التعريفات** الرسمية.
🔹 إنشاء **فواتير صيانة** احترافية.
🔹 **البحث عن قطع الغيار** بالوصف.
...
تفضل، كيف يمكنني خدمتك اليوم؟`,
  ...
  const saveToHistory = (msg: Message) => {
    ...
    alert('تم حفظ التشخيص في سجل المهندس بنجاح ✅');
  };
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 custom-scrollbar">
          ...
          <div key={msg.id} className={`flex flex-col w-full ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
            <div className={`relative max-w-[90%] md:max-w-[75%] rounded-[2rem] p-5 shadow-sm transition-all hover:shadow-md ${
              ...
              {msg.type === 'image' && msg.metadata?.imageUrl && (
                <img src={msg.metadata.imageUrl} alt="Uploaded" className="max-w-full rounded-2xl mb-3 max-h-64 object-cover border border-white/20" />
              )}
              <div 
                className="whitespace-pre-wrap leading-relaxed text-sm md:text-base font-medium"
        ...
        {isLoading && (
          <div className="flex justify-start">
             <div className="bg-white p-4 rounded-full shadow-sm border border-slate-100 flex gap-1.5">
      ...
      <div className="p-4 md:p-6 bg-white border-t border-slate-100 shadow-2xl">
        <div className="flex items-center gap-3 max-w-4xl mx-auto">
          ...
          <input
            ...
            placeholder="صف العطل أو اسأل عن موديل..." className="flex-1 bg-slate-50 border-none rounded-2xl px-5 py-3.5 focus:ring-2 focus:ring-blue-600 focus:outline-none font-medium shadow-inner"
          />
